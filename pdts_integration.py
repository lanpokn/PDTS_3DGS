...
class PDTSViewSelector:
    ...
    """
    def __init__(self, device, x_dim=13, bootstrap_iterations=2000, lambda_diversity=0.5, opacity_reset_interval=3000, reset_recovery_period=500):
        self.device = device
        ...
        # New parameters for handling opacity reset cycles
        self.opacity_reset_interval = opacity_reset_interval  # When opacity resets happen
        self.reset_recovery_period = reset_recovery_period    # Random sampling period after reset
        ...
        print(f"[PDTS] Opacity reset handling: Every {opacity_reset_interval} iterations, random sampling for {reset_recovery_period} iterations")

    def _is_in_recovery_period(self):
        """Check if we're in a recovery period after an opacity reset."""
    ...
    def select_views(self, viewpoint_pool, num_selected=16, num_candidates=256):
        ...
        if not self.should_use_network():
            ...
            # Determine the reason for random sampling
        if self.iteration <= self.bootstrap_iterations:
            mode = "bootstrap"
        elif self._is_in_recovery_period():
            mode = "recovery"
        else:
            mode = "fallback"
        
        return [viewpoint_pool[i] for i in selected_indices], mode
